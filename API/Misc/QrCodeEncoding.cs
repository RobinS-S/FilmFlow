...
namespace FilmFlow.API.Misc
{
    public class QrCodeEncoding
    {
        ...
        public static async Task<byte[]> GenerateTicket(string code, string title, string time, string subtitle, string bodyText)
        {
            ...
            int padding = 20;
            int titleHeight = 50 + padding;
            int subtitleHeight = 50 + padding;
            int timeHeight = 50 + padding;
            int bodyTextHeight = 50 * 4 + padding;
            int imageWidth = image.Width / 2;
            int imageHeight = (int)(((double)imageWidth / image.Width) * image.Height);
